  ...
  for (let index = 0; index < calculation.lines.length; index += 1) {
    ...
    const lineEntity =
      ...
      em.create(SalesOrderLine, {
        order,
        // id must be assigned client-side (not left to the DB `gen_random_uuid()` default) —
        // setRecordCustomFields() below reads lineEntity.id before any flush occurs.
        id: sourceLine.id ?? randomUUID(),
...
async function replaceQuoteLines(
  ...
  for (let index = 0; index < calculation.lines.length; index += 1) {
    ...
    const lineEntity = em.create(SalesQuoteLine, {
      // SalesQuoteLine.id uses a DB-assigned default (`gen_random_uuid()`), which stays
      // undefined on the entity until flush — setRecordCustomFields() below needs a real
      // recordId immediately, so generate it client-side here.
      id: randomUUID(),
      quote,
      ...entityInput,
      status: statusValue,
...
async function replaceOrderLines(
  ...
  for (let index = 0; index < calculation.lines.length; index += 1) {
    ...
    const lineEntity = em.create(SalesOrderLine, {
      // SalesOrderLine.id uses `defaultRaw: 'gen_random_uuid()'` — a DB-assigned default that
      // stays undefined on the entity until flush. setRecordCustomFields() below needs a real
      // recordId immediately (no flush happens between persist and this call), so the id must be
      // generated client-side here, same as replaceOrderAdjustments() does for adjustment rows.
      id: randomUUID(),
      order,
      ...entityInput,
      reservedQuantity: "0",
