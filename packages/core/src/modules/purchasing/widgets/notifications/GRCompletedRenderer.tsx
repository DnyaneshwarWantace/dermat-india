...
import { PackageCheck, ExternalLink, Loader2, X } from 'lucide-react'
...
export function GRCompletedRenderer({
  notification,
  onAction,
  onDismiss,
  actions = [],
  ...
  const viewAction = actions.find((a) => a.id === 'view-goods-receipt') ?? actions[0] ?? null
  ...
  const receiptNumber = notification.bodyVariables?.receiptNumber ?? notification.titleVariables?.receiptNumber
  const itemCount = notification.bodyVariables?.itemCount
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-1">
        ...
        <div className="text-xs leading-4 text-muted-foreground">
          ...
          {itemCount ? (
            <>
              <span aria-hidden="true" className="mx-1 text-text-disabled">&middot;</span>
              <span className="whitespace-nowrap font-medium text-foreground">{itemCount} items received</span>
            </>
          ) : null}
        </div>
        ...
        <div className="mt-2 flex items-center gap-2">
          ...
          >
            ...
            {t('purchasing.notifications.grCompleted.viewGoodsReceipt', 'View Receipt')}
...
export default GRCompletedRenderer
