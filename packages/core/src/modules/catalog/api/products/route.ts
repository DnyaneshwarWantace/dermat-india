...
async function decorateProductsAfterList(
  ...
  try {
    ...
    const variants = await findWithDecryption(
      ...
      CatalogProductVariant,
      { product: { $in: productIds }, deletedAt: null, ...scope },
      { fields: ["id", "product", "sku", "name", "weightValue", "weightUnit", "isDefault", "isActive", "metadata"] },
      scope,
    );
    ...
    for (const variant of variants) {
      ...
      list.push({
        ...
        name: variant.name,
        sku: variant.sku,
        weight_value: variant.weightValue,
    ...
    for (const item of items) {
      ...
      const prodVariants = variantsByProduct.get(id) ?? [];
      item.variants = prodVariants;
      item.variant_count = prodVariants.length;
