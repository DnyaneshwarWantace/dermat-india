...
// Products created without a typed SKU used to save with sku=null — the code column then
// rendered blank in the products list, and any later reference to "the product" had nothing
// human-readable to key off. Auto-assign <prefix>00001, <prefix>00002, ... scoped per
// tenant/org, so a product always has a stable, unique, readable code even if nobody types
...
async function resolveAutoSkuPrefix(
  ...
  try {
    const configService = ctx.container.resolve(
      "moduleConfigService",
    ) as import("../../configs/lib/module-config-service").ModuleConfigService;
    const value = await configService.getValue<string>(
      CATALOG_SETTINGS_MODULE_ID,
      AUTO_SKU_PREFIX_KEY,
      { scope: { tenantId, organizationId }, defaultValue: AUTO_SKU_PREFIX_DEFAULT },
    );
    const trimmed = typeof value === "string" ? value.trim() : "";
    return trimmed.length ? trimmed : AUTO_SKU_PREFIX_DEFAULT;
  } catch {
    return AUTO_SKU_PREFIX_DEFAULT;
  }
}
...
async function generateNextProductSku(
  ...
  const lastSku = rows[0]?.sku ?? null;
  const lastNumber = lastSku
    ? Number.parseInt(lastSku.slice(prefix.length), 10)
    : 0;
  const nextNumber = (Number.isFinite(lastNumber) ? lastNumber : 0) + 1;
  return `${prefix}${String(nextNumber).padStart(AUTO_SKU_PAD, "0")}`;
}
...
> = {
  ...
  async execute(rawInput, ctx) {
    ...
    const { translate } = await resolveTranslations();
    const now = new Date();
    const resolvedSku = parsed.sku?.trim()
      ? parsed.sku.trim()
      : await generateNextProductSku(em, ctx, parsed.organizationId, parsed.tenantId);
