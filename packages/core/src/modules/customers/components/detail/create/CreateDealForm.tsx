...
export function CreateDealForm({ returnTo, initialValues }: CreateDealFormProps) {
  ...
  const handleSubmit = React.useCallback(async () => {
    ...
    try {
      ...
      if (values.productId) custom.product_id = values.productId
      ...
      <div className="mt-6 grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,1fr)_330px]">
        <div className="space-y-4">
          ...
          <DealProductSection
            ...
            onProductChange={(next) => patch({ productId: next })}
            companyId={values.companyIds[0]}
            disabled={isSubmitting}
          />
        </div>
