...
export type DealCustomAttributesProps = {
  ...
  /** Custom field ids (e.g. `cf_product_id`) rendered by a dedicated section elsewhere on the form and hidden here to avoid duplicate controls. */
...
export function DealCustomAttributes({
  ...
  const visibleFields = excludeFieldIds?.length
    ? fields.filter((field) => !excludeFieldIds.includes(field.id))
    : fields
