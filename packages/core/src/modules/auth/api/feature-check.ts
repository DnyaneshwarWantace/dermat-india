...
const featureCheckResponseSchema = z.object({
  ...
  userEmail: z.string().optional().describe('Email of the authenticated user, when available on the session'),
})
...
export async function POST(req: Request) {
  ...
  if (!features.length) {
    return NextResponse.json({ ok: true, granted: [], userId: auth.sub, userEmail: auth.email })
  }
  ...
  if (ok) {
    return NextResponse.json({ ok: true, granted: features, userId: auth.sub, userEmail: auth.email })
  }
  ...
  return NextResponse.json({ ok: false, granted, userId: auth.sub, userEmail: auth.email })
}
