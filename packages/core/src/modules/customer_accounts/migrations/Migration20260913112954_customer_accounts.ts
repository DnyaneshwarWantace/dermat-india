...
export class Migration20260913112954_customer_accounts extends Migration {
  ...
  override up(): void | Promise<void> {
    ...
    this.addSql(`alter table "customer_role_acls" add constraint "customer_role_acls_role_id_foreign" foreign key ("role_id") references "customer_roles" ("id");`);

    this.addSql(`alter table "customer_user_acls" add constraint "customer_user_acls_user_id_foreign" foreign key ("user_id") references "customer_users" ("id");`);

    this.addSql(`alter table "customer_user_email_verifications" add constraint "customer_user_email_verifications_user_id_foreign" foreign key ("user_id") references "customer_users" ("id");`);

    this.addSql(`alter table "customer_user_password_resets" add constraint "customer_user_password_resets_user_id_foreign" foreign key ("user_id") references "customer_users" ("id");`);

    this.addSql(`alter table "customer_user_roles" add constraint "customer_user_roles_role_id_foreign" foreign key ("role_id") references "customer_roles" ("id");`);
    this.addSql(`alter table "customer_user_roles" add constraint "customer_user_roles_user_id_foreign" foreign key ("user_id") references "customer_users" ("id");`);

    this.addSql(`alter table "customer_user_sessions" add constraint "customer_user_sessions_user_id_foreign" foreign key ("user_id") references "customer_users" ("id");`);

    this.addSql(`alter table "domain_mappings" drop constraint if exists "domain_mappings_hostname_normalized_chk";`);
    this.addSql(`alter table "domain_mappings" alter column "dns_failure_reason" type text using ("dns_failure_reason"::text);`);
    this.addSql(`alter table "domain_mappings" alter column "tls_failure_reason" type text using ("tls_failure_reason"::text);`);
    this.addSql(`alter table "domain_mappings" alter column "created_at" drop default;`);
    this.addSql(`alter table "domain_mappings" add constraint "domain_mappings_provider_check" check ("provider" in ('traefik'));`);
    this.addSql(`alter table "domain_mappings" add constraint "domain_mappings_status_check" check ("status" in ('pending', 'verified', 'active', 'dns_failed', 'tls_failed'));`);
  ...
  override down(): void | Promise<void> {
    ...
    this.addSql(`alter table "domain_mappings" drop constraint if exists "domain_mappings_provider_check";`);
    this.addSql(`alter table "domain_mappings" drop constraint if exists "domain_mappings_status_check";`);
    this.addSql(`alter table "domain_mappings" drop constraint if exists "domain_mappings_hostname_normalized_chk";`);
    this.addSql(`alter table "domain_mappings" alter column "dns_failure_reason" type varchar(500) using ("dns_failure_reason"::varchar(500));`);
    this.addSql(`alter table "domain_mappings" alter column "tls_failure_reason" type varchar(500) using ("tls_failure_reason"::varchar(500));`);
    this.addSql(`alter table "domain_mappings" alter column "created_at" set default now();`);
