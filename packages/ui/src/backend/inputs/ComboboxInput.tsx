...
export function ComboboxInput({
  ...
  const [popoverRect, setPopoverRect] = React.useState<{ top: number; left: number; width: number } | null>(null)
  ...
  // Portal the popup to <body> so it always escapes ancestor `overflow`
  // clipping (e.g. a horizontally-scrollable table wrapper) instead of being
  // cut off by a scroll container it happens to render inside.
  React.useEffect(() => {
    if (!listboxVisible) {
      setPopoverRect(null)
      return
  ...
  return (
    <div className="relative w-full" ref={wrapperRef}>
      ...
      {listboxVisible && popoverRect && typeof document !== 'undefined' && createPortal(
        <div
          className="fixed z-popover rounded-md border border-input bg-popover p-2 shadow-md max-h-48 sm:max-h-60 overflow-auto"
