...
function ShellBrandLogo({
  ...
  const isCustomLogo = Boolean(logo?.src && logo.src !== '/open-mercato.svg')
  if (!isCustomLogo) {
    return (
      <div
        className={`${mobile ? 'h-7 w-7 text-xs' : 'h-8 w-8 text-sm'} rounded-lg bg-primary text-primary-foreground font-extrabold flex items-center justify-center shrink-0 shadow-xs tracking-tight`}
      >
        DI
      </div>
    )
